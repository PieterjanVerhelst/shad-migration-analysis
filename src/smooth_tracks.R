...
distance_matrix <- read_csv("./data/external/distancematrix_shad.csv")
...
# For each shad, the nearest stations are found by `get_nearest_stations()` and saved in a list, `near_stations`
...
# For each shad, smoothing is applied by calling function `get_timeline`
tracks <- purrr::map(shads_all, 
                     function(shad) 
                       get_timeline(data, 
                                    proxy_stations = near_stations_all,
                                    shad = shad, verbose = FALSE))
...
sum(residency$counts) # Check that number of counts/detections equals the length of the original dataset 'data'.
