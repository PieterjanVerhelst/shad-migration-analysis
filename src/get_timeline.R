#' Retrieve smooth shad tracking time series along several stations.
#' 
#' This function smoothes an shad tracking time series along several stations (receivers).
...
#' @param shad (character) a code identifying the transmitter implanted in the shad. 
...
get_timeline <- function(df, 
  ...
  df %<>% filter(tag_serial_number == shad)
  # find start
  t <- min(df$date_time)
  ...
  while (t < max_t) {
    ...
    # shad is at a new "not near" station or detected after time limit
    if ((!new_station %in% near_stations & new_station != station_name) |
      ...
      timeline %<>% bind_rows(
        ...
        stringsAsFactors = FALSE))
      if (isTRUE(verbose)) {
        print(paste(shad, "is at", t, "at station:", station_name))
      }
    } else{
  ...
  timeline$tag_serial_number <- shad
